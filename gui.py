...
from PyQt6.QtGui import QAction, QColor, QBrush, QIcon, QKeySequence, QPainter, QPixmap, QShowEvent, QHideEvent
...
from hotkeys_service import HotkeyService, DisabledHotkeyService
from platform_factory import create_platform_services
from platform_runtime import app_support_dir, is_macos, logs_dir
from system_ops import SystemOps


# Persist settings under the per-user app-support dir so they survive working
# directory changes (e.g. when launched from Finder/Start menu).
CONFIG_FILE = os.path.join(app_support_dir(), "settings.json")
_LEGACY_CONFIG_FILE = "settings.json"
...
def _migrate_legacy_settings_if_needed() -> None:
    """First-run conservative migration: copy ./settings.json into app_support_dir.

    Keeps the legacy file in place to avoid surprising users who rely on it.
    """
    try:
        ...
        logger.info("Migrated legacy settings.json into %s", CONFIG_FILE)
    except Exception:
        logger.exception("Failed to migrate legacy settings.json")
...
def _load_settings_dict() -> Dict:
    """Load settings.json without instantiating SettingsWindow.

    Used at startup so the detector router can resolve strategy before
    the Qt windows are built. Failures yield an empty dict; the router
    then falls back to its UNIVERSAL default.
    """
    try:
        ...
        logger.exception("Failed to read settings for detector strategy.")
        return {}
...
class SignalManager(QObject):
    ...
    # Emitted from any thread to bounce a hotkey action onto the Qt main thread.
    # Payload is a short action name: "mic" | "loopback" | "both" | "stop".
...
class AutohideProgressStrip(QFrame):
    ...
    def _on_tick(self) -> None:
        try:
            ...
            logger.debug("toast_strip_tick_failed", exc_info=True)
    ...
    def _apply_geometry(self) -> None:
        ...
        self._fill.setFixedWidth(max(2, int(w * frac)))
...
class RecorderBarWindow(QWidget):
    ...
    def _build_ui(self):
        ...
        self.btn_rec.setFixedWidth(44)
        self.btn_stop.setFixedWidth(44)
        self.btn_hide.setFixedWidth(0)
        self.btn_hide.setVisible(False)
        self.btn_settings.setFixedWidth(0)
        ...
        self.lbl_timer.setMinimumWidth(44)
    ...
    def update_meter(self, metrics: Dict[str, float]):
        ...
        bars = "▁▂▃▄▅▆▇"[min(7, int(peak * 8))]
        self.lbl_meter.setText(bars * 4)
...
class MeetingPromptWindow(QWidget):
    ...
    def _build_ui(self):
        ...
        self.lbl_sub = QLabel("Возможно, пора начать запись.")
        self.lbl_sub.setWordWrap(True)
        self.lbl_sub.setStyleSheet("color: #aab4c4; font-size: 10px; font-weight: 500;")
        ...
        self.btn_dismiss.setMinimumWidth(96)
        self.btn_record.setMinimumWidth(96)
        self.btn_dismiss.setMinimumHeight(28)
        self.btn_record.setMinimumHeight(28)
...
class StopRecordingPromptWindow(QWidget):
    """Toast asking the user whether to stop the ongoing recording.

    Mirrors the visual language of :class:`MeetingPromptWindow` so the
    user recognises it as part of the same flow. Primary action ("Stop")
    is on the right, mirroring the start prompt's "Record" button.
    """
    ...
    def _build_ui(self):
        ...
        self.btn_continue.setMinimumWidth(96)
        self.btn_stop.setMinimumWidth(96)
        self.btn_continue.setMinimumHeight(28)
        ...
        self.setStyleSheet(
            ...
            #stopPromptPanel #toastProgressTrack {
                background: rgba(0, 0, 0, 0.35);
                border: none;
                border-top-left-radius: 11px;
                border-top-right-radius: 11px;
            }
            #stopPromptPanel #toastProgressFill {
                background: qlineargradient(x1:0,y1:0,x2:1,y2:0,
                    stop:0 #3dd9a0, stop:0.45 #5eb0ff, stop:1 #b18cff);
                border: none;
                border-top-left-radius: 11px;
...
class SettingsWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        group_call = QGroupBox("Call Detection")
        layout_call = QFormLayout()
        # User-facing values are localized; internal storage uses the
        # canonical English keys so settings.json stays language-stable.
        self.combo_call_end = QComboBox()
        self.combo_call_end.addItem("Спрашивать", "Ask")
        self.combo_call_end.addItem("Автоматически", "Auto")
        self.combo_call_end.addItem("Игнорировать", "Ignore")
        layout_call.addRow("Когда звонок завершился:", self.combo_call_end)
        group_call.setLayout(layout_call)
        layout.addWidget(group_call)
    ...
    def load_settings(self):
        ...
        try:
            ...
            call_end = str(data.get("call_end_action", "Ask"))
            call_end_idx = self.combo_call_end.findData(call_end)
            if call_end_idx < 0:
                call_end_idx = 0
            self.combo_call_end.setCurrentIndex(call_end_idx)
    ...
    def save_settings(self):
        ...
        try:
            ...
            logger.exception("Failed to read previous settings for diff.")
            previous = {}
        try:
            ...
            logger.info(
                "settings_saved | keys_changed=[%s] | path=%s",
                ",".join(changed_keys),
                _mask_home(CONFIG_FILE),
            )
    ...
    def get_settings(self) -> Dict:
        return {
            ...
            "call_end_action": self.combo_call_end.currentData() or "Ask",
...
class TrayApplication(QObject):
    def __init__(
        ...
        # Independent visibility flags so start and stop prompts don't
        ...
        # Build detector via the strategy router (env or settings.json,
        # defaulting to UNIVERSAL). settings_window isn't built yet, so
        # we re-load settings.json directly via SettingsWindow's recipe.
        ...
        self.signals.hotkey_triggered.connect(self._on_hotkey_triggered)
        self._detector_eval_lock = threading.Lock()
        self._detector_eval_running = False

        # Generated tray icons live in the writable app-support dir;
        # ``resource_path`` may point into a read-only PyInstaller bundle.
        self.icon_idle_path = os.path.join(app_support_dir(), "icon_idle.png")
        self.icon_rec_path = os.path.join(app_support_dir(), "icon_rec.png")
        self.generate_icons()
        ...
        self.stop_prompt_window = StopRecordingPromptWindow()
        self.stop_prompt_window.stop_clicked.connect(self.handle_stop_prompt_stop)
        self.stop_prompt_window.continue_clicked.connect(self.handle_stop_prompt_continue)
    ...
    def _log_startup_summary(self) -> None:
        ...
        try:
            ...
            logger.exception("Could not read settings for startup summary.")
            settings = {}
        try:
            strategy = resolve_strategy(_load_settings_dict()).value
        except Exception:
            strategy = "unknown"
        call_end = settings.get("call_end_action", "Ask") or "Ask"
        logger.info(
            "tray_ready | detector_strategy=%s | call_end_action=%s | mic_id=%s | output_folder=%s | format=%s",
            strategy,
            call_end,
            settings.get("device_id") or "",
            _mask_home(settings.get("output_folder", "")),
            settings.get("format", ""),
        )
    ...
    def _position_stop_prompt(self):
        screen = self.app.primaryScreen().availableGeometry()
        x = screen.right() - self.stop_prompt_window.width() - 18
        y = screen.top() + 18
        self.stop_prompt_window.move(max(0, x), max(0, y))
    ...
    def generate_icons(self):
        if not os.path.exists(self.icon_idle_path):
            ...
            painter.end()
            try:
                if not pix.save(self.icon_idle_path):
                    logger.warning("QPixmap.save returned False for %s", self.icon_idle_path)
            except Exception:
                logger.exception("Failed to write idle tray icon to %s", self.icon_idle_path)
        if not os.path.exists(self.icon_rec_path):
            ...
            painter.end()
            try:
                if not pix.save(self.icon_rec_path):
                    logger.warning("QPixmap.save returned False for %s", self.icon_rec_path)
            except Exception:
                logger.exception("Failed to write recording tray icon to %s", self.icon_rec_path)
    ...
    def _normalize_hotkey(combo: str) -> str:
        ...
        # Order doesn't matter for collision detection, so sort.
    ...
    def register_hotkeys(self):
        ...
        try:
            for key, action in actions:
                ...
                if normalized in seen:
                    conflicts.append(f"{combo} ({key} duplicates {seen[normalized]})")
                    logger.warning(
                        "Skipping duplicate hotkey: %s for %s; already bound to %s",
                        combo,
                        key,
                        seen[normalized],
                    )
                    continue
                seen[normalized] = key
                # Emit via the SignalManager so the callback always runs on the
                # Qt main thread regardless of which thread fired it.
        ...
        if conflicts:
            self.tray_icon.showMessage(
                "Hotkeys",
                "Duplicate hotkey ignored: " + "; ".join(conflicts),
                QSystemTrayIcon.MessageIcon.Warning,
                3500,
            )

    def _on_hotkey_triggered(self, action: str):
        """Dispatcher slot run on the Qt main thread."""
    ...
    def on_tray_activated(self, reason):
        # Single left-click: when idle (no live recorder), start recording using
        # the user's configured tray-click mode. While recording, single click
        # toggles the floating bar so the user can see/stop it.
        if reason == QSystemTrayIcon.ActivationReason.Trigger:
            is_idle = self.state == "idle" and not (self.recorder and self.recorder.is_alive())
            if is_idle:
                try:
                    settings = self.settings_window.get_settings()
                    mode = self._resolve_record_mode_from_settings(settings)
                except Exception:
                    logger.exception("Tray click: failed to resolve record mode.")
                    mode = self.last_mode if self.last_mode in ("mic", "loopback", "both") else "mic"
                logger.info("tray_click | action=start_%s", mode)
                self.start_recording(mode)
                return
            logger.info("tray_click | action=toggle_bar")
            self._toggle_bar()
        elif reason in (
    ...
    def on_detection_decision(self, decision):
        if (
            ...
            or decision.should_prompt_start
            or decision.should_prompt_stop
            or decision.auto_stop
        ):
            context_hash = hashlib.sha1(
                (decision.context_key or "").encode("utf-8")
            ).hexdigest()[:10]
            # Dedup at the detector level: only emit on state transitions.
            should_log = True
            log_method = getattr(self.detector, "should_log_decision", None)
            if callable(log_method):
                try:
                    should_log = bool(log_method(decision))
                except Exception:
                    logger.exception("detector.should_log_decision raised; falling back to log-always.")
                    should_log = True
            if should_log:
                logger.info(
                    "meeting_detector | score=%s | rules=%s | decision=%s | ctx_hash=%s | call_pid=%s | call_proc=%s | loop_rms=%.4f | loop_peak=%.4f | sustain=%.2f",
                    decision.score,
                    ",".join(decision.matched_rules),
                    decision.reason,
                    context_hash,
                    decision.call_pid or "",
                    decision.call_process_name or "",
                    decision.debug.get("loopback_rms", 0.0),
                    decision.debug.get("loopback_peak", 0.0),
                    decision.debug.get("loopback_sustain", 0.0),
                )
            # Raw context_key may contain window-title PII; keep at DEBUG only.
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    "meeting_detector raw_context | ctx_hash=%s | context=%s",
                    context_hash,
                    decision.context_key,
                )

        is_recording = bool(self.recorder and self.recorder.is_alive())
        if not is_recording and decision.should_prompt_start and not self.prompt_visible:
            self.show_meeting_prompt(decision)
            return
        if is_recording and (decision.should_prompt_stop or decision.auto_stop):
            self._handle_call_end_signal(decision)

    def _handle_call_end_signal(self, decision):
        if self.stop_prompt_visible:
            return
        action = self._call_end_action_setting()
        if decision.auto_stop or action == "Auto":
            logger.info("Auto-stopping recording on call end (action=%s)", action)
            self.stop_recording()
            return
        if action == "Ignore":
            return
        self.show_stop_prompt(decision)

    def _call_end_action_setting(self) -> str:
        try:
            settings = self.settings_window.get_settings()
            value = str(settings.get("call_end_action", "Ask"))
        except Exception:
            value = "Ask"
        if value not in ("Ask", "Auto", "Ignore"):
            value = "Ask"
        return value

    def show_meeting_prompt(self, decision=None):
        if self.recorder and self.recorder.is_alive():
            return
        friendly = ""
        proc = ""
        if decision is not None:
            friendly = (getattr(decision, "app_display_name", "") or "").strip()
            proc = (getattr(decision, "call_process_name", "") or "").strip()
            if not friendly and proc:
                friendly = self._friendly_app_name(proc)
        if friendly:
            self.prompt_window.lbl_title.setText(f"Звонок в {friendly}?")
        else:
            self.prompt_window.lbl_title.setText("Похоже, у вас начался звонок")
        self.prompt_window.lbl_sub.setText("Возможно, пора начать запись.")
        ...
        logger.info(
            ...
            proc or "",
            getattr(decision, "call_pid", "") or "",
        )

    def show_stop_prompt(self, decision=None):
        self._position_stop_prompt()
        self.stop_prompt_window.show()
        ...
        if decision is not None:
            proc = (getattr(decision, "call_process_name", "") or "").strip()
            pid_value = getattr(decision, "call_pid", "") or ""
        logger.info(
            "prompt_stop_shown | call_proc=%s | call_pid=%s",
            proc,
            pid_value,
        )
    ...
    def handle_prompt_record(self):
        logger.info("prompt_choice | prompt=start | choice=record")
    ...
    def handle_prompt_dismiss(self):
        logger.info("prompt_choice | prompt=start | choice=dismiss")
        self.dismiss_meeting_prompt()
        self.detector.set_cooldown_dismiss()

    def handle_stop_prompt_stop(self):
        logger.info("prompt_choice | prompt=stop | choice=stop")
        self.dismiss_stop_prompt()
        self.stop_recording()

    def handle_stop_prompt_continue(self):
        logger.info("prompt_choice | prompt=stop | choice=continue")
        self.dismiss_stop_prompt()
        # 30s PID cooldown so a transient end-of-call snapshot doesn't
        # immediately re-prompt while the user is still talking.
        try:
            self.detector.set_cooldown_dismiss()
        except Exception:
            logger.exception("Failed to set dismiss cooldown on stop-prompt continue.")

    @staticmethod
    def _friendly_app_name(proc: str) -> str:
        name = proc.strip().lower()
        if not name:
            return ""
        mapping = {
            ...
            "yandextelemost.exe": "Яндекс Телемост",
            "chrome.exe": "браузере",
            "msedge.exe": "браузере",
            "firefox.exe": "браузере",
            "yandex.exe": "браузере",
            "brave.exe": "браузере",
            "opera.exe": "браузере",
        }
    ...
    def start_recording(self, mode="loopback"):
        ...
        if output_log:
            logger.info(
                "session_log_path | path=%s",
                _mask_home(output_log),
            )
        ...
        if mode in ("loopback", "both") and not self._has_default_loopback():
            ...
            self._maybe_show_blackhole_hint()
            return
    ...
    def exit_app(self):
        recorder_was_active = bool(self.recorder and self.recorder.is_alive())
        logger.info(
            "app_stop | recorder_was_active=%s | reason=user",
            recorder_was_active,
        )
        ...
        try:
            ...
            logger.exception("Failed to stop meeting detector on exit.")
        if self.recorder:
            try:
                # Block briefly so the output file is finalized before we quit.
                self.recorder.stop(wait=True, timeout=8.0)
            except Exception:
                logger.exception("Failed to stop recorder cleanly on exit.")
        self.app.quit()

    # --- macOS one-time hints (QW3) -----------------------------------------

    @staticmethod
    def _hint_already_shown(marker_path: str) -> bool:
        return os.path.exists(marker_path)

    @staticmethod
    def _record_hint_shown(marker_path: str) -> None:
        try:
            with open(marker_path, "w", encoding="utf-8") as f:
                f.write("shown\n")
        except Exception:
            logger.exception("Failed to write macOS hints marker: %s", marker_path)

    def _maybe_show_macos_hotkey_hint(self) -> None:
        if not is_macos():
            return
        if not isinstance(self.hotkey_service, DisabledHotkeyService):
            return
        if self._hint_already_shown(_MAC_HOTKEY_HINT_MARKER):
            return
        self.tray_icon.showMessage(
            "Hotkeys unavailable",
            "Global hotkeys are not available on macOS; use the floating panel.",
            QSystemTrayIcon.MessageIcon.Information,
            4500,
        )
        self._record_hint_shown(_MAC_HOTKEY_HINT_MARKER)

    def _maybe_show_blackhole_hint(self) -> None:
        if not is_macos():
            return
        if self._hint_already_shown(_MAC_BLACKHOLE_HINT_MARKER):
            return
        self.tray_icon.showMessage(
            "System audio capture",
            "Install BlackHole and select it as the system output to record system audio.",
            QSystemTrayIcon.MessageIcon.Information,
            5000,
        )
        self._record_hint_shown(_MAC_BLACKHOLE_HINT_MARKER)
