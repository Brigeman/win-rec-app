...
def _ctx_hash(context_key: str) -> str:
    """Stable short hash for log lines (PII-free)."""
    return hashlib.sha1((context_key or "").encode("utf-8")).hexdigest()[:10]
...
class DetectionDecision:
    # ``should_prompt_start`` is the renamed legacy ``should_prompt`` flag.
    # The two new ``should_prompt_stop`` / ``auto_stop`` fields are only set
    # by the universal detector; legacy always leaves them False so callers
    ...
    def should_prompt(self) -> bool:
        return self.should_prompt_start
...
class LegacyMeetingDetector:
    def __init__(
        ...
        # Per-instance dedup tracking so callers can emit one line per
        # state transition rather than once per evaluation tick.
    ...
    def set_cooldown_dismiss(self):
        ...
        logger.info(
            "cooldown_set | strategy=legacy | seconds=%.1f | ctx_hash=%s | kind=dismiss",
            float(self.rules.dismiss_cooldown_seconds),
            _ctx_hash(self.last_evaluated_context or self.last_prompt_context),
        )
        # Allow the same context to prompt again once the cooldown elapses.
        self.last_prompt_context = ""

    def set_cooldown_post_stop(self):
        self._set_context_cooldown(self.rules.post_stop_cooldown_seconds)
        logger.info(
            "cooldown_set | strategy=legacy | seconds=%.1f | ctx_hash=%s | kind=post_stop",
            float(self.rules.post_stop_cooldown_seconds),
            _ctx_hash(self.last_evaluated_context or self.last_prompt_context),
        )
        # Allow the same context to prompt again once the cooldown elapses.
        self.last_prompt_context = ""
    ...
    def should_log_decision(self, decision: "DetectionDecision") -> bool:
        """Return True only when reason changed or prompt flag flipped.

        Avoids per-tick log spam while still recording every state
        transition (e.g. ``threshold_not_met`` → ``threshold_met``).
        """
    ...
    def evaluate(self, is_recording: bool, mic_rms: float = 0.0) -> DetectionDecision:
        # TODO: mic_rms wiring depends on idle mic probe; tracked separately.
        # Today callers pass 0.0 because we don't have a free mic probe outside
        # recording, so rules conditioned on positive mic_rms never fire.
        now = time.time()
        if is_recording:
            # Legacy detector never produces stop signals; just exit early.
        ...
        # NOTE: rules conditioned on a positive ``mic_rms`` are intentionally
        # disabled while we don't have an idle mic probe. Re-enable once the
        # idle mic level is wired in (see TODO above).
        ...
        if should_prompt:
            ...
            logger.info(
                "prompt_start | strategy=legacy | reason=%s | score=%s | ctx_hash=%s | rules=%s | sustain=%.2f | rms=%.4f | peak=%.4f",
                reason,
                score,
                _ctx_hash(context_key),
                ",".join(matched),
        ...
        return DetectionDecision(
            should_prompt_start=should_prompt,
            score=score,
    ...
    def _is_context_on_cooldown(self, context_key: str, now: float) -> bool:
        ...
        if now >= expiry:
            ...
            # The cooldown just lapsed for this context; clear the
            # "already prompted" guard so the same context can prompt again.
            if self.last_prompt_context == context_key:
                self.last_prompt_context = ""
            return False
        return True
...
# Backwards-compatible alias: external scripts/tests may still import
# ``MeetingDetector``. The router-aware code should construct detectors
# via :func:`detector_router.create_detector` instead.
MeetingDetector = LegacyMeetingDetector
